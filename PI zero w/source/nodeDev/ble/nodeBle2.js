...
function charCallback(data, isNotification){ // receive data and create one line reply per command
 ...
 function postProcessData(dataIn,hooks){
	var data = dataIn;
	for (var i = 0; i < hooks.length; i++){
		...
		switch(pross.type){
			case "delete" : {
				delete data[pross.input];
				break;
			}
			case "merge" : {
				var merged = "";
				pross.input.forEach(function(element){
					merged = merged.concat(data[element].toString());
					delete data[element];
				});
				data[pross.output] = merged;
				break;
			}
			default: {
...
function constructDataFromLine(data,callback){
	...
	if (commandDataSet[tempoID].hooks !== undefined){
		console.log(currentDataBlock.data)
		console.log(commandDataSet[tempoID].hooks)
		currentDataBlock.data = postProcessData(currentDataBlock.data,commandDataSet[tempoID].hooks);
		console.log(currentDataBlock.data)
	}
...
function sendACommand(){
	...
	mldpCharCharacteristic.write(sendCommand,false); // send command ... data will be received in charCallback
}
