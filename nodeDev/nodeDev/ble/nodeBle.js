...
function isInTimeSpan(timeSpan){
	...
	if (timeSpan.active == true){
		div = 1.0 / 60;
		hour = date.getHours() + (date.getMinutes() * div);
		strt = parseInt(timeSpan.start.hour,10) + (parseInt(timeSpan.start.min,10) * div);
		endt = parseInt(timeSpan.stop.hour,10) + (parseInt(timeSpan.stop.min,10) * div);
		if (strt < endt){
